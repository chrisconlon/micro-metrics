...
\pause 
  ...
  Note there is already an important assumption embedded in this setup, the stable unit treatment value assumption (\alert{SUTVA}). 
...
\begin{frame}
  ...
  Guido has a good discussion on this. Suggests following motivations: 
  \begin{itemize}
    ...
    \item \textit{All} comparisons involve comparing treated to untreated units. Absent RCT, its up to researcher to invstigate which comparisons to emphasize
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Could easily also compute $$\mu_w(x) = \beta_w'x$$
    \item Key point is that this estimator can be viewed as a missing data problem, where predictions are computed using regression. 
  \end{itemize} 
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{enumerate}
      ...
      \item treated and control observations are similar
    \end{enumerate}
    \item First is untestable, but in practice predictions are often sensitive to functional form
...
\begin{frame}
  ...
  \begin{itemize}
  \item Linh To's JMP: 
  \item Question: by how much did "marginal" mothers extend leave when benefits extended in Denmark
  \item Convincing RD: restrict to sample already pregnant when law announced 
  \item Challenge: Only see mothers in one group or the other (2 pre, 3 post)
