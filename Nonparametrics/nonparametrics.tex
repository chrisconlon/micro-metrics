...
\begin{frame}{Nonparametric Regression}
  ...
  \end{itemize}
  \item One option is to pick a parametric function form $y=f(x)$. But often hard to think about how sensible these assumptions are (and what the impose on the problem)
%  \item Another option is to extend density discussion we just had to two dimensions using local weighted averages 
...
\item A natural alternative is to use kernels or local smoothers
\end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \[
{\hat m}_n(\mathbf{x_0})=\frac{\sum_{i=1}^n y_i
K\left(\frac{x-x_i}{h}\right)}{\sum_{i=1}^n
K\left(\frac{x-x_i}{h}\right)}.
\]
