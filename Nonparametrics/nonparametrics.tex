...
\begin{frame}{Use Lowess Plot to Motivate DD}
\begin{figure}[htbp]
\begin{center}
...
\begin{frame}{Bootstrap: OLS}
 \begin{itemize}
 	...
 	\item OLS: $\hat \beta_{ols} = [\sum^N_{j=1} X_j X_j']^{-1}[\sum^N_{j=1} X_j Y_j] $
 	...
 	$$ V(\beta_{ols}) = \frac{1}{B}\sum_b^B (\hat B_{ols,b} - \bar \beta_{ols,b}) \cdot  (\hat B_{ols,b} - \bar \beta_{ols,b})' $$
 \end{itemize}
\end{frame}
...
\begin{frame}[allowframebreaks]{Bootstrap: Variants}
  ...
  \begin{itemize}
  	\item First estimated $\beta$ by OLS
  	\item Get residuals $\hat \epsilon_i$ 
