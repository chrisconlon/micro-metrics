...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%TWBLCDF(x,l,u,a,b) calculates a truncated Weibull cdf with scale parameter 
...
%
%When b=1, this function computes a truncated exponential cdf with
%parameter a.
% 
%See also TLOGNCDF, TNORMCDF, TRAYLCDF, TLOGNPDF, TNORMPDF, TRAYLPDF, 
%TWBLPDF
...
elseif l==0 & isfinite(u)   % if the right tail only is truncated
    y=wblcdf(x,a,b)*(1/wblcdf(u,a,b));
elseif l>0 & isinf(u)       % if the left tail only is truncated
    y=(wblcdf(x,a,b) - wblcdf(l,a,b))*(1/(1-wblcdf(l,a,b)));
elseif l>0 & isfinite(u)    %if both tails are truncated
    y=(wblcdf(x,a,b) - wblcdf(l,a,b))*(1/(wblcdf(u,a,b)-wblcdf(l,a,b)));
end
