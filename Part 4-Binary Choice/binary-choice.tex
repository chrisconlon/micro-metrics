...
\begin{enumerate}
\item Run the LPM with instruments (Suggested by MHE).
\item Specify the distribution of errors in first and second stage and do MLE (\texttt{ivprobit} in STATA).
...
\end{quote}
\begin{itemize}
\item Mentions heteroskedasticity of error (which is binomial) but does not address the violation of the first LSA.
\end{itemize}
\end{frame}
