...
\begin{frame}{Mean and Variance of $\hat{f}(x_0)$}
  ...
  So, unsurprisingly, the bias is decreasing in $h$, and the variance is \textit{increasing} in $h$
\end{frame}
...
\begin{frame}{Mean Integrated Square Error}
  \begin{itemize}
    \item Start with the local performance at $x_0$
...
\begin{frame}{Silverman's Rule of Thumb}
  ...
  \item Just do it with $\sigma=s$ empirical dispersion of the $x_i$'s , or something more robust/slightly less smooth:
...
%TK ADD CROSS VALIDATION SLIDE FROM THIS 
